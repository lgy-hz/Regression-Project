...
In this section, used R code below to find out out-liner in box-plots and removed them from the data set. From the box-plot, out-liner in hp, wt,qsec and carb can be found. Moving further to identify the out-liners.
...
project<-project[-c(31,15,16,17,9),]
...
The average MPG of automatic transmission is `r mean(project[project$am==0,"mpg"])`. And the average MPG of manual transmission is `r mean(project[project$am==1,"mpg"])`. The cooperation between transmission type and MPG is `r cor(project$am,project$mpg)`. At this level of relationship analysis, transmission type and MPG shows a strong correlation and manual transmission have MPG **`r mean(project[project$am==1,"mpg"])-mean(project[project$am==0,"mpg"])`**mpg than auto transmission.
...
model1<-summary(lm(project$mpg~ project$am))
...
model2<-summary(lm(project$mpg~  project$am + project$drat+ project$qsec + project$hp))
model2
...
model3<-summary(lm(project$mpg~  project$am + project$qsec +  project$hp))
model3
...
```{r }
library(ggplot2)
project$residual <- model1$residuals # add the residuals to the frame

  ggplot(project) + aes(x=mpg, y=residual, color=am) +
...
pairs(data=project, mpg ~ wt + cyl + disp + am + hp+ drat+ qsec+ vs,
    panel=panel.smooth, main="Relationships between MPG and other variable")

```


###Residual plot for multivariate Variable Regression

```{r}
project$residual2 <- model2$residuals # add the residuals to the frame

  ggplot(project) + aes(x=mpg, y=residual2, color=am) +
    xlab("Miles per gallon") + 
    ylab("residual") +
    geom_point(shape=19, size=5, alpha=1/2, aes(color=am)) +
    ggtitle("Residuals-2")
...
text<-data.frame(project$am,project$drat,project$qsec)
cor(text)
